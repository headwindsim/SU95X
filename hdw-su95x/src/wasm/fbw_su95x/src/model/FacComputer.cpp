...
void FacComputer::step()
{
  ...
  if (FacComputer_U.in.sim_data.computer_running) {
    ...
    if (rtb_V_ias >= 60.0F) {
      if (rtb_y_i) {
        rtb_Y_i = FacComputer_U.in.bus_inputs.elac_1_bus.rudder_pedal_position_deg.Data;
      } else if (rtb_Memory) {
        rtb_Y_i = FacComputer_U.in.bus_inputs.elac_2_bus.rudder_pedal_position_deg.Data;
      } else {
        rtb_Y_i = FacComputer_P.Constant_Value_n;
      }

      rtb_beDot = (rtb_beDot * rtb_beDot * 0.6125 * 122.0 / (70000.0 * Vtas) * 3.172 * (FacComputer_P.Gain_Gain_h *
        rtb_Y_i) * 3.1415926535897931 / 180.0 + (rtb_phi * 3.1415926535897931 / 180.0 * (9.81 / Vtas) - rtb_r *
    ...
    if (rtb_alpha > FacComputer_P.Saturation_UpperSat) {
      rtb_Y_i = FacComputer_P.Saturation_UpperSat;
    } else if (rtb_alpha < FacComputer_P.Saturation_LowerSat) {
      rtb_Y_i = FacComputer_P.Saturation_LowerSat;
    } else {
      rtb_Y_i = rtb_alpha;
    }

    FacComputer_LagFilter(rtb_Y_i, FacComputer_P.LagFilter2_C1, FacComputer_U.in.time.dt, &rtb_Switch1_a,
    ...
    if (rtb_alt >= FacComputer_P.CompareToConstant_const) {
      rtb_Y_i = std::sqrt(std::pow((std::pow(rtb_Switch1_a * rtb_Switch1_a * 0.2 + 1.0, 3.5) - 1.0) * (rtb_p_s_c /
        1013.25) + 1.0, 0.2857142857142857) - 1.0) * 1479.1;
    } else {
      rtb_Y_i = FacComputer_P.Constant_Value_l;
    }

    rtb_Switch1_a = std::fmax(std::fmax(rtb_Y_i, FacComputer_P.Vmcl_Value), rtb_Switch4_f);
    ...
    if (rtb_V_ias > FacComputer_P.Saturation_UpperSat_j) {
      rtb_Y_i = FacComputer_P.Saturation_UpperSat_j;
    } else if (rtb_V_ias < FacComputer_P.Saturation_LowerSat_c) {
      rtb_Y_i = FacComputer_P.Saturation_LowerSat_c;
    } else {
      rtb_Y_i = rtb_V_ias;
    }

    rtb_Gain_a = FacComputer_P.DiscreteDerivativeVariableTs_Gain_l * rtb_Y_i;
