...
void FacComputer::step()
{
  ...
  if (FacComputer_U.in.sim_data.computer_running) {
    ...
    if (rtb_V_ias >= 60.0F) {
      ...
      Vtas = (a * a * 0.6125 * 122.0 / (70000.0 * Vtas) * 3.172 * (FacComputer_P.Gain_Gain_h * rtb_y_f5_0) *
    ...
    if ((rtb_alpha_floor_inhib == 0) && (rtb_mach < 0.6)) {
      ...
      if ((rtb_y_f5_0 > rtb_Switch1_a + std::fmin(std::fmax(rtb_Switch_b, static_cast<real_T>(rtb_Switch4_a)), 0.0)) &&
          rtb_Memory) {
    ...
    FacComputer_CalculateV_alpha_max(static_cast<real_T>(rtb_V_ias), rtb_y_f5_0, rtb_Switch1_a, rtb_Switch_b,
      &rtb_handleIndex);
    FacComputer_RateLimiter(look2_binlxpw(static_cast<real_T>(rtb_mach), rtb_Switch4_f,
    ...
    FacComputer_CalculateV_alpha_max(static_cast<real_T>(rtb_V_ias), rtb_y_f5_0, rtb_Switch1_a, rtb_Switch_b, &rtb_Y_p);
    FacComputer_RateLimiter(look2_binlxpw(static_cast<real_T>(rtb_mach), rtb_Switch4_f,
    ...
    FacComputer_MATLABFunction_d(rtb_Memory, rtb_Compare_b, rtb_DataTypeConversion_gv, rtb_OR_k,
      rtb_DataTypeConversion_do, rtb_y_c != 0U, &rtb_handleIndex);
    FacComputer_MATLABFunction2(look1_binlxpw(rtb_handleIndex, FacComputer_P.uDLookupTable3_bp01Data,
      FacComputer_P.uDLookupTable3_tableData, 5U), look1_binlxpw(rtb_handleIndex, FacComputer_P.uDLookupTable2_bp01Data,
    ...
    absx = 1.0 / rtb_Switch4_f;
    if (absx > FacComputer_P.Saturation3_UpperSat) {
      absx = FacComputer_P.Saturation3_UpperSat;
    } else if (absx < FacComputer_P.Saturation3_LowerSat) {
      absx = FacComputer_P.Saturation3_LowerSat;
    }

    rtb_Switch4_f = look2_binlxpw(look2_binlxpw(absx, static_cast<real_T>(rtb_DataTypeConversion2),
      ...
      FacComputer_P.uDLookupTable2_bp02Data, FacComputer_P.uDLookupTable2_tableData_h,
      FacComputer_P.uDLookupTable2_maxIndex, 28U);
    absx = FacComputer_P.Gain_Gain_km * rtb_alt;
    if (absx > FacComputer_P.Saturation2_UpperSat) {
      absx = FacComputer_P.Saturation2_UpperSat;
    } else if (absx < FacComputer_P.Saturation2_LowerSat) {
      absx = FacComputer_P.Saturation2_LowerSat;
    }

    absx = look2_binlxpw(absx, rtb_Switch4_f, FacComputer_P.uDLookupTable3_bp01Data_p,
                         FacComputer_P.uDLookupTable3_bp02Data, FacComputer_P.uDLookupTable3_tableData_c,
                         FacComputer_P.uDLookupTable3_maxIndex, 18U);
    if (absx > FacComputer_P.Saturation1_UpperSat_ok) {
      absx = FacComputer_P.Saturation1_UpperSat_ok;
    } else if (absx < FacComputer_P.Saturation1_LowerSat_o) {
      absx = FacComputer_P.Saturation1_LowerSat_o;
    }

    if ((!rtb_AND1) && rtb_y_j && rtb_Switch_i_idx_1) {
      FacComputer_DWork.takeoff_config = rtb_handleIndex;
    } else if (FacComputer_DWork.takeoff_config != rtb_handleIndex) {
      FacComputer_DWork.takeoff_config = -1.0;
    }

    if (rtb_handleIndex == 0.0) {
      rtb_handleIndex = 1.28;
    } else if (FacComputer_DWork.takeoff_config != -1.0) {
      rtb_handleIndex = 1.13;
    } else {
      rtb_handleIndex = 1.23;
    }

    FacComputer_RateLimiter_f(rtb_handleIndex, FacComputer_P.RateLimiterGenericVariableTs_up,
    ...
    if (rtb_alt >= FacComputer_P.CompareToConstant_const) {
      rtb_handleIndex = std::sqrt(std::pow((std::pow(absx * absx * 0.2 + 1.0, 3.5) - 1.0) * (rtb_p_s_c / 1013.25) + 1.0,
        0.2857142857142857) - 1.0) * 1479.1;
    } else {
      rtb_handleIndex = FacComputer_P.Constant_Value_l;
    }

    rtb_Switch1_a = std::fmax(std::fmax(rtb_handleIndex, FacComputer_P.Vmcl_Value), rtb_Switch4_f);
    ...
    rtb_handleIndex = look1_binlxpw(rtb_Switch_b, FacComputer_P.uDLookupTable_bp01Data_b,
    ...
    absx = std::fmin(rtb_DataTypeConversion2 / 2205.0 * 1.25 + 169.0, 215.0) + std::fmax(rtb_alt - 20000.0, 0.0) /
      1000.0;
    FacComputer_MATLABFunction_f(&FacComputer_U.in.bus_inputs.sfcc_own_bus.slat_flap_system_status_word,
    ...
    rtb_Switch4_j = std::fmin(std::fmin(rtb_Switch_b, std::sqrt(std::pow((std::pow(FacComputer_P.Constant1_Value_p *
    ...
    FacComputer_Y.out.flight_envelope.alpha_filtered_deg = rtb_y_f5_0;
    ...
    FacComputer_Y.out.flight_envelope.v_ls_kn = rtb_handleIndex;
    ...
    FacComputer_Y.out.flight_envelope.v_man_kn = absx;
    ...
    FacComputer_Y.out.bus_outputs.v_ls_kn.Data = static_cast<real32_T>(rtb_handleIndex);
    ...
    FacComputer_Y.out.bus_outputs.v_man_kn.Data = static_cast<real32_T>(absx);
